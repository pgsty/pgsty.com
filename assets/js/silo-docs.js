...
(function () {
  ...
  // 底部语言选择弹层（右上角另有一键直切链接）。
  function initLangMenu() {
    var root = document.querySelector('.silo-lang');
    if (!root) return;
    var btn = root.querySelector('[data-silo-lang-toggle]');
    var pop = root.querySelector('.silo-lang__pop');
    if (!btn || !pop) return;
